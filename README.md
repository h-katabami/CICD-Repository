...
5. `terraform/state-backend` で `terraform init` と `terraform apply` を実行
6. 出力された `terraform_state_bucket`, `aws_role_arn_dev`, `aws_role_arn_prod` を kawagoe リポジトリの Variables/Secrets に登録
