...
3. 既存案件リポジトリ（例: kawagoe）から reusable workflow を呼び出す（`project_suffix` を指定）か、ワークフロー内容をコピーして利用
...
7. 出力された `terraform_state_bucket`, `aws_role_arn_dev`, `aws_role_arn_prod` を利用する（固定値を workflow に直書きする運用なら、案件リポジトリの Variables/Secrets 登録は不要）
